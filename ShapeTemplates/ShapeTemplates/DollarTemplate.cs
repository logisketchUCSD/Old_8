...
namespace ShapeTemplates
{
    ...
    public class DollarTemplate
    {
        ...
        public DollarTemplate RecognizeSymbol(List<DollarTemplate> templates, ref double score)
        {
        ...
        public Dictionary<DollarTemplate, double> RecognizeSymbol(List<DollarTemplate> templates)
        {
            if (templates.Count == 0)
                return null;

            Dictionary<double, DollarTemplate> recoResults = new Dictionary<double, DollarTemplate>();

            foreach (DollarTemplate T in templates)
            {
                double theta = 0.0;
            ...
            Dictionary<DollarTemplate, double> outputResults = new Dictionary<DollarTemplate, double>(recoResults.Count);
        ...
        private double EuclideanDistanceAtBestAngle(List<AveragePoint> points, DollarTemplate T, double thetaA, double thetaB, double thetaDelta, ref double theta)
        {
        ...
        private double distanceAtBestAngle(List<AveragePoint> points, DollarTemplate T, double thetaA, double thetaB, double thetaDelta, ref double theta)
        {
        ...
        private double distanceAtAngle(List<AveragePoint> avgPoints, DollarTemplate T, double theta)
        {
        ...
        /// <returns></returns>
        private double EuclideanDistanceAtAngle(List<AveragePoint> avgPoints, DollarTemplate T, double theta)
        {
            List<Point> points = GetPoints(avgPoints);
            List<Point> newPoints = rotateBy(points, theta);
        ...
        public static List<DollarTemplate> LoadTemplates(string dir)
        {
            /*
            ...
            */
            ...
            List<DollarTemplate> templates = new List<DollarTemplate>();
            ...
            foreach (string file in files)
            {
                ...
                foreach (DollarTemplate temp in templates)
                {
                    if (temp.Name == name)
                    {
                        foundTemplate = true;
                ...
                if (!foundTemplate)
                    templates.Add(new DollarTemplate(points, name));
            }
    ...
    class AveragePoint
    {
        ...
        private void FindAvgsAndStdDevs()
        {
            ...
            {
                m_StdDevX = Compute.StandardDeviation(m_AllX);
                m_StdDevY = Compute.StandardDeviation(m_AllY);
                if (m_StdDevX < 1f)
                    m_StdDevX = 1f;
                if (m_StdDevY < 1f)
                    m_StdDevY = 1f;

...
def empty_csv():
    ...
    return [['actual/observed', 'line', 'ellipse', 'rectangle', 'precision', 'recall', 'accuracy'],
            ['line', 0, 0, 0, '', '', ''],
            ['ellipse', 0, 0, 0, '',''],
            ['rectangle', 0, 0, 0, '', '']]
...
def anova_summary(recognizer, userset):
    """anova summarization"""
    lines = []
    lines.append(["Test Set", "Iso Train", "ER Train", "Process Train", " "]*3)
    for run in runs:
        line = []
    ...
    for user in users:
        for run in runs:
            if user in os.listdir("%s/%s" % (run, recognizer)):
                line = [user,run]
                for s in usersets:
                    a = csv_to_array("%s/%s/%s/%s/iso-er.csv" %
                                       (run,recognizer,user,s))
                    b = csv_to_array("%s/%s/%s/%s/iso-process.csv" %
                                       (run,recognizer,user,s))
                    csv = add_csv(a,b)
...
def recognizer_anova_summary(userset, test_set):
    lines = []
    lines.append(["Run-Training", "Bullseye", " "]*3)
    for run in runs:
        line = []

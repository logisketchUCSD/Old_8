...
activities = ["basic", "er", "process"]

runs = [ "Run"+str(x) for x in xrange(1, 26) ]
users = [ "USER"+str(x) for x in xrange(0,32) ]
...
user-specific training data"""
    ...
    userlist = os.listdir(input_folder)
    # make sure we're only getting USERX directories
    for x in usersets:
        if x in userlist: userlist.remove(x)
    # sum up the confusion matrices for all the users
    for userdir in ["%s\\%s\\%s" % (input_folder, user, userset) for user in userlist]:
    ...
    for user in users:
        for run in runs:
            if user in os.listdir("%s\\%s" % (run, recognizer)):
                line = [user,run]
                for s in usersets:
                    a = csv_to_array("%s\\%s\\%s\\%s\\basic-er.csv" %
                                       (run,recognizer,user,s))
                    b = csv_to_array("%s\\%s\\%s\\%s\\basic-process.csv" %
                                       (run,recognizer,user,s))
                    csv = add_csv(a,b)
